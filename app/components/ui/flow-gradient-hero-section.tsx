...
class TouchTexture {
  ...
  update() {
    // Skip canvas redraw and GPU upload when there is no active trail
...
class GradientBackground {
  ...
  init() {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      // Reduced from 6 gradient centers to 4 — cuts per-pixel GPU work by ~33%
      fragmentShader: `
        uniform float uTime, uSpeed, uIntensity, uGrainIntensity, uGradientSize, uColor1Weight, uColor2Weight;
        uniform vec2 uResolution;
        uniform vec3 uColor1, uColor2, uColor3, uColor4, uDarkNavy;
...
class App {
  ...
  constructor(container: HTMLElement) {
    ...
    // antialias off (no effect on shader output), pixel ratio fixed at 1,
    // powerPreference low-power to reduce heat/battery drain
    ...
    this.visibilityHandler = () => {
      if (document.hidden) {
        ...
        this.clock.getDelta(); // discard the large delta accumulated while hidden
        this.tick();
      }
    };
    this.init();
  ...
  init() {
    ...
    // Debounce resize — avoid expensive geometry rebuilds on every pixel of drag
    ...
    // Pause rAF loop when tab is not visible
